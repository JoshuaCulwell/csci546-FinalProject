...
class MLPTransition(nn.Module):
    def __init__(
        ...
        mlp_dim = hidden_dim + input_embedding_dim
        self.network = MLP(
            in_dim = mlp_dim,
            out_dim = hidden_dim,
            hidden_dim = width,
        ...
        self.input_embedding = (
            nn.Linear(in_dim, input_embedding_dim)
            if input_embedding_dim != in_dim else
            None
        )

    def forward(self, hidden_state, x):
        if self.input_embedding: x = self.input_embedding(x)
        return self.network(torch.cat([hidden_state, x], dim=-1))  
...
class DiffusionTransition(nn.Module):
    def __init__(
        ...
        denoiser_in_dim = hidden_dim * 2 + input_embedding_dim + timestep_dim
        self.denoiser = MLP(
...
class VariableTransitionRNN(nn.Module):
    ...
    def forward(self, input_sequence, initial_hidden_state=None):
        batch_size, sequence_length, input_embedding_dim = input_sequence.shape
